...
export default function TroubleList({ query }: { query: string }) {
  const { tickets, isLoading, deleteTicket } = useTrouble();
  ...
  // Logic: Ambil aktivitas terakhir (Update atau Created)
  const getLastActivity = (ticket: Trouble) => {
    // Cek jika ada updates (History)
    if (ticket.updates && ticket.updates.length > 0) {
      // Ambil update terakhir
      const lastUpdate = ticket.updates[
        ticket.updates.length - 1
      ] as TroubleHistory;

      // Join array reporters menjadi string
      const userNames = lastUpdate.reporters
        ? lastUpdate.reporters.join(', ')
        : 'Unknown';

      return {
        user: userNames,
        date: lastUpdate.date, // Menggunakan field 'date' dari TroubleHistory
        isEdit: true,
      };
    }

    // Jika belum ada update, ambil data pembuatan
    const creator =
  ...
  const filteredTickets = useMemo(() => {
    const data: Trouble[] = tickets ? [...tickets] : [];
    let result = data;
            ...
            <tbody className="divide-y divide-[#E6E0E9]">
              {paginatedTickets.length > 0 ? (
                paginatedTickets.map((t) => {
                  ...
                  // Ambil gambar pertama dari array images yang valid
                  const thumbnail =
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-start gap-4">
                          ...
                          <div className="max-w-[240px] flex flex-col gap-1">
                            ...
                            {/* Link Detail */}
                            <Link
                              href={`/operational/tickets/trouble/detail/${t.id}`}
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span
                          className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold uppercase tracking-wide border ${getStatusColor(
                            t.status,
                          )}`}
                        >
                          {t.status}
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        {t.priority && (
                          <span
                            className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-md text-[11px] font-semibold border ${getPriorityColor(
                              t.priority,
                            )}`}
                          >
                            <AlertCircle size={12} />
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end items-center gap-2">
                          ...
                          {/* Link Edit */}
                          <Link
                            href={`/operational/tickets/trouble/edit/${t.id}`}
